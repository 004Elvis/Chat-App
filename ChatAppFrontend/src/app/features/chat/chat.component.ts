    ...
    this.signalRService.reconnected$.subscribe(async () => {
      ...
      if (currentRoom) {
        try {
          ...
          console.error('Could not rejoin room after reconnect:', error);
        }
      }
    });
  }
    ...
    this.chatService.createRoom(name, true, []).subscribe({
      next: room => {
        this.chatService.getRoom(room.id).subscribe({
          ...
          error: err => console.error('Could not load newly created room:', err)
        });
      },
      error: err => console.error('Could not create group:', err)
    });
  }
